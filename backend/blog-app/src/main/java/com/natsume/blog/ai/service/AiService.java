...
public class AiService {
    ...
    public AiService(AiConfigService aiConfigService, VectorSearchService vectorSearchService) {
    ...
    private List<RetrievedDoc> retrieve(String query, int topK) {
        try {
            return vectorSearchService.retrieve(query, topK).stream().map(d -> {
                RetrievedDoc r = new RetrievedDoc();
                r.setPostId(d.getPostId());
